# Step 17 - Progression affinee
# -------------------------------------------------------------------
# OBJECTIF PEDAGOGIQUE:
# - Stabiliser la boucle de progression complete.
# - Clarifier les infos de HUD pour le joueur/debutant.
#
# NOUVEAUTE:
# - Le HUD affiche aussi le nombre de murs.
# - Le message de niveau est plus explicite.
# - A chaque niveau, le coffre precedent est remplace (ouvert ou non).
#
# NOTE TEMPORAIRE:
...
def generate_monsters(level, player_pos, walls):
    ...
    for _ in range(count):
        ...
        if level >= 10:
            ...
            monster_type = random.choice(["slime", "gobelin"])
            hp = 4 + level // 2
            atk = 1 + level // 8
        else:
            monster_type = "slime"
        ...
        monsters.append(
            {
                ...
                "move_cooldown": 0.0,
                "attack_cooldown": 0.0,
                # Plus le niveau monte, plus delay diminue.
                "move_delay": max(0.28, 0.95 - 0.06 * level),
                "hit_flash": 0.0,
            }
        )
...
def build_level_state(player):
    ...
    if player["level"] >= 10:
        ...
        msg = f"Niveau {player['level']}: Vague de 2 monstres."
    else:
...
def draw_monsters(screen, monsters, sprites, hit_sprites):
    for monster in monsters:
        ...
        if monster["type"] == "slime":
            ...
            # Le boss n'a pas de sprite dedie ici -> rectangle colore.
            sprite = None
            fallback = (170, 60, 120)
...
def draw_hud(screen, font, player, monsters, message):
    ...
    line_1 = (
        ...
        f"Monstres: {alive_count} | "
        f"Murs: {wall_count_for_level(player['level'])}"
    )
...
pygame.display.set_caption("Step 17 - Progression affinee")
...
while running:
    ...
    # Regle de progression de l'etape 16
    alive_count = sum(1 for monster in monsters if monster["alive"])
    if alive_count == 0:
        # IMPORTANT: le coffre precedent est implicitement supprime ici,
        # car build_level_state retourne un nouveau coffre.
        # C'est exactement la regle demandee.
        #
        # On borne a 10 pour garder le boss comme palier max.
...
# ------------------------------------------------------------
# EXERCICE GUIDE - STEP 17 (PROGRESSION AFFINEE)
# ------------------------------------------------------------
# Objectif de l'exercice:
# - Lire le HUD et verifier la logique de progression.
#
# Etape 1:
# - Joue jusqu'au niveau suivant.
# - Verifie les infos HUD: niveau, PV, murs, monstres.
#
# Etape 2:
# - Trouve le texte de message de niveau.
# - Reformule-le avec tes propres mots.
#
# Etape 3:
# - Confirme qu'a chaque niveau un nouveau coffre est cree
#   (l'ancien est remplace automatiquement).
# ------------------------------------------------------------
