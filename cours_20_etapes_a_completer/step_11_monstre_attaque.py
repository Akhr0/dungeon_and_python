# Step 11 - Le monstre attaque le joueur
# -------------------------------------------------------------------
# OBJECTIF PEDAGOGIQUE:
# - Ajouter la notion de degats et de cooldown d'attaque.
# - Introduire un premier "etat de defaite" tres simple.
#
# NOUVEAUTE:
...
def update_monsters(monsters, player, chest, walls, dt):
    ...
    for monster in monsters:
        ...
        # Nouvelle partie importante:
        # si adjacent, on attaque le joueur.
        # ============================================================
# >>>>>>>>>>>>>> ZONE A COMPLETER PAR L'ELEVE >>>>>>>>>>>>>>>
# >>> Ecris ton code ENTRE les deux balises START / END
# >>> START CODE ELEVE
        # (Attendu: si monstre adjacent et cooldown fini -> retirer des PV au joueur)
...
def draw_hud(screen, font, player, message):
    ...
    screen.blit(font.render("Bouger: fleches/ZQSD | E: coffre", True, COLOR_TEXT), (10, hud_y + 34))
...
pygame.display.set_caption("Step 11 - Monstre attaque")
...
message = "Le monstre peut maintenant te blesser."
...
while running:
    ...
    if player["hp"] <= 0:
        message = "Tu n'as plus de PV (etat game over arrive a l'etape suivante)."
...
# ------------------------------------------------------------
# EXERCICE GUIDE - STEP 11 (ATTAQUE MONSTRE)
# ------------------------------------------------------------
# Objectif de l'exercice:
# - Observer l'impact du cooldown et des degats.
#
# Etape 1:
# - Change MONSTER_ATTACK_DELAY (0.9 -> 1.4).
# - Verifie que le monstre frappe moins souvent.
#
# Etape 2:
# - Trouve la ligne qui retire des PV au joueur.
# - Passe les degats de 1 a 2.
# - Verifie que la vie descend plus vite.
#
# Etape 3:
# - Remets les valeurs par defaut pour garder l'equilibre.
# ------------------------------------------------------------
